...
```ts
import { useListAllCommunities, useGetMyPosts, useCreateNewChat, useUpdateMyProfile } from '@dataconnect/generated/react';
// The types of these hooks are available in react/index.d.ts

const { data, isPending, isSuccess, isError, error } = useListAllCommunities();

const { data, isPending, isSuccess, isError, error } = useGetMyPosts();

const { data, isPending, isSuccess, isError, error } = useCreateNewChat(createNewChatVars);
