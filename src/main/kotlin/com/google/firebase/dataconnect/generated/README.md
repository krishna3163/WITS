...
is the query named "GetMyPosts",
which can be accessed via the [ExampleConnector.getMyPosts] property.


An example of the property for a mutation
is the mutation named "CreateNewChat",
which can be accessed via the [ExampleConnector.createNewChat] property.
...
For example, the "GetMyPosts" query has no variables
and can be executed via the
[ExampleConnector.getMyPosts]
property as follows:

```kotlin
val connector = ExampleConnector.instance
val queryResult = connector.getMyPosts.execute()
println("GetMyPosts query returned: ${queryResult.data}")
```
...
For example, the "CreateNewChat" mutation has 1 required variable ("chatType")
and can be executed via the [ExampleConnector.createNewChat]
property as follows:

```kotlin
val connector = ExampleConnector.instance
val mutationResult = connector.createNewChat.execute(chatType="quux")
println("CreateNewChat mutation returned: ${mutationResult.data}")
```
...
For example, the "UpdateMyProfile" mutation has 4 optional variables ("bio", "location", "profilePictureUrl", and "displayName")
and can be executed via the [ExampleConnector.updateMyProfile]
property as follows:

```kotlin
val connector = ExampleConnector.instance
val mutationResult = connector.updateMyProfile.execute {
  bio = "baz"
  location = "fred"
  profilePictureUrl = "qux"
  displayName = "garply"
}
println("UpdateMyProfile mutation returned: ${mutationResult.data}")
```
